	...
	for (i = ARRAY_SIZE(pm_min_residency) - 1; i >= 0; i--) {
		...
		if ((ticks == K_FOREVER) ||
			...
			case 1: /* Standby mode */
				...
				if (ticks != K_FOREVER) {
					/* TBD: Ideally we'd like to set a
					 ...
					 * ClockP_start) does not have
					 * sub-millisecond accuracy, wakeup
					 * would at WAKEDELAYSTANDBY rounded
					 * up to the next ms ahead of the next
					 * timeout.
					 * This also has the implication that
					 * SYS_PM_MIN_RESIDENCY_DEEP_SLEEP_1
