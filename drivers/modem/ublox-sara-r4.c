...
static int offload_poll(struct pollfd *fds, int nfds, int msecs)
{
	...
	for (i = 0; i < nfds; i++) {
		...
		/* If vtable maches, then it's modem socket. */
...
static bool offload_poll_takeover(struct zsock_pollfd *fds, int nfds)
{
	int i;
	void *obj;
...
NET_SOCKET_REGISTER(ublox_sara_r4, AF_UNSPEC, offload_is_supported,
		    offload_socket, offload_poll_takeover, offload_poll);
