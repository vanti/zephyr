...
{
	/* TBD: Should we indicate here that this device started a transaction
	 * that should not be
	 * interrupted by putting the SoC into the deep sleep mode. NRF driver
	 * seems to do it. But there is currently no call to device_busy_check.
	 */
	/* device_busy_set(dev); */
...
static void uart_cc13xx_cc26xx_irq_tx_disable(struct device *dev)
{
	...
	/* TBD: Should we assume here that the transaction is over, and it
	 * is okay to enter the deep sleep mode if needed? Though there is
	 * currently no call to device_busy_check.
	 */
...
static void uart_cc13xx_cc26xx_irq_rx_enable(struct device *dev)
{
	 ...
	 * standby.
	 *
	 * TBD: Should we use device_busy_set instead of Power_setConstraint
	 * once idle is doing checking it?
	 */
...
#ifdef CONFIG_SYS_POWER_MANAGEMENT
	/*
	 * TBD: Should we use device_busy_clear instead of
	 * Power_releaseConstraint once idle is doing checking it?
	 */
	if (get_dev_data(dev)->rx_constrained) {
...
#ifdef CONFIG_SYS_POWER_MANAGEMENT
	unsigned long intStatus = UARTIntStatus(get_dev_conf(arg)->regs, true);

	if ((intStatus & UART_INT_TX) && data->tx_constrained) {
		Power_releaseConstraint(PowerCC26XX_DISALLOW_STANDBY);
...
/*
 *  ======== postNotifyFxn ========
 *  Called by Power module when waking up the CPU from Standby. The uart needs
